...
logger = logging.getLogger('mapping uploader logger')
logger.setLevel(logging.DEBUG)
handler = TimedRotatingFileHandler('./logs/mapping_uploader.log', when='midnight', interval=1, backupCount=30)
...
def delete_index(index_name):
    ...
    url = f'http://{Config.ES_HOST}/{index_name}'
...
def update_index(index_name, index_data):
    ...
    url = f'http://{Config.ES_HOST}/{index_name}/_mapping'
...
def delete_schema(dirpath):
    logger.info(f'Delete schema in directory: {dirpath}')
...
def create_schema(dirpath):
    logger.info(f'Create schema in directory: {dirpath}')
    for dirpath, dirnames, filenames in os.walk(dirpath):
        for filename in filenames:
